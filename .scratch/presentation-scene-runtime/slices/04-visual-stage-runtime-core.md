...
- [ ] Active scene state includes camera map, active camera ID, model records, and root.
- [ ] `loadDefinition()` or equivalent accepts a validated definition and atomically promotes only after all declared models load.
- [ ] Scene definitions with omitted or empty `models` are valid runtime inputs.
- [ ] Scene definitions with omitted or empty `lights` receive no runtime fallback lights.
- [ ] Failed model loading preserves the previous active root, active camera, and visibility.
- [ ] Stale load completion cannot replace newer active scene state.
- [ ] Promotion order adds pending root and replaces active state before disposing old state.
- [ ] Successful loads automatically show the Visual Stage.
- [ ] Failed or stale loads do not change visibility.
- [ ] Named **Presentation Cameras** are built as perspective cameras, and `setCamera(cameraId)` switches instantly.
- [ ] Layout updates active camera aspect before rendering.
- [ ] Supported lights are constructed from definition data.
- [ ] Multiple flat models can load into one pending root.
- [ ] Optional fit behavior uses Slice 03 helper behavior.
- [ ] Initial model animation starts when declared and exactly found; missing initial animation is non-fatal.
- [ ] `play(modelId, clipName)` targets one model and returns false with diagnostics for missing model or exact clip.
- [ ] Resource disposal uses Slice 03 helper behavior.
- [ ] `dispose()` is terminal and repeated dispose calls are no-ops.
